...
*   **Two-Level Scaling for High-Precision Representation:** NVFP4 uses two distinct scaling factors, which is its most critical feature. To understand this, let's define two terms:
    *   A **Tensor** is a large, multi-dimensional array of numbers that holds the model's weights or activations. These are the core data structures in a neural network, and their scale can be immense. Here are some concrete examples based on the models described in the paper:
        *   **Weight Tensor:** In the 12B parameter model, the Feed-Forward Network (FFN) layers project from the model's hidden dimension (5120) to the FFN's inner dimension (20480). This single weight tensor contains over 104 million floating-point numbers.
        *   **Activation Tensor:** Activations are the outputs of a neural network layer that become the inputs to the next. For the 8B model, with a batch size of 768 and a sequence length of 8192, the activation tensor flowing between layers has a shape of `[768, 8192, 4096]` (batch, sequence, model dimension). Before being used in a matrix multiplication, this is typically reshaped into a massive 2D tensor of shape `[6291456, 4096]`.
    *   A **Block** is a small, fixed-size chunk of a tensor. In NVFP4, a block is a group of just 16 contiguous numbers. So, the enormous weight and activation tensors above would be partitioned into thousands or millions of these tiny blocks for quantization.
    ...
    1.  **Coarse, Per-Tensor Scaling (FP32):** First, a single scaling factor is calculated for the *entire tensor* based on its absolute maximum value. This factor performs a rough, global adjustment, bringing all the values in the tensor into a more manageable intermediate range.
    2.  **Fine-Grained, Per-Block Scaling (FP8):** After the first scaling is applied, the tensor is divided into thousands of small 16-element blocks. For *each* of these blocks, a second, more precise scaling factor is calculated. This local factor makes a fine-tuned adjustment, perfectly mapping the 16 values to the extremely limited range of FP4.
